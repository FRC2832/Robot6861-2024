...
/**
 * This class wraps the REV ColorSensorV3 class, as the color sensor uses I2C
 * reads that sometime lock the Rio.
 * See sampleUsage() to see how to use the sensor to match to a color. There is
 * no periodic function with this class as the reads are done on a background
 * thread.
 */
public class REVColorSensor {
    ...
    public REVColorSensor(Port i2cPort) {
        ...
        // move color sensor read to separate thread since it sometimes locks up reading
        // I2C
    ...
    /**
     * Get the currently seen color. Works best when within 2 inches and
    ...
    protected void sampleUsage() {
        ...
        // a unknown target is needed to give the color matcher a place to go when
        // empty,
    ...
    private class ReadColorSensorThread extends Thread {
        ...
        public void run() {
            while (true) {
                ...
                proximity = colorSensor.getProximity() / 2047.0;
