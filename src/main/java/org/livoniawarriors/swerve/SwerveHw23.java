...
public class SwerveHw23 implements ISwerveDriveIo {
    // measuring the robot, we got 11114 counts/90*, the theoretical amount is
    // 10971.428/90* (150/7:1 gear ratio, 2048 counts/rev)
    ...
    public SwerveHw23() {
        ...
        for (TalonFX motor : driveMotor) {
            ...
            // old value of 0.03 means if we had 1m/s error for 1 second, add 0.03V to the
            // PID (or 1.705 counts of 1023)
            // unknown why we need 2/3, but then the math works...
            // allConfigs.slot0.kI = 0.03 * Constants.CTRE_P_RES / COUNTS_PER_METER *
            // (2./3);
    ...
    public void setCornerState(int wheel, SwerveModuleState swerveModuleState) {
        ...
        double turnVolts = -turnPid[wheel].calculate(Math.toRadians(correctedAngle[wheel]),
